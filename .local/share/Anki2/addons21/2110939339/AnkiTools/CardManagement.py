...
class CardManagement:
    ...
    def note_ids_from_card_ids(items):
        return mw.col.db.list(f"select distinct nid from cards where id in {ids2str(items)}")
    ...
    def garbageCollectionRegular(settings: Settings):
        ...
        gcProcessedListeningCards, lcWithClozeForEnd, lcWithoutClozeForEnd, clozeCardsForStart, clozeCardsForEnd = (
            CardManagement._getManagedRegularCardAndNoteIds(settings)
        )
        ...
        if settings.promptBeforeCardManagementAction:
            message = "Card management would:\n\n"
            if len(lcWithClozeForEnd) != 0:
                message += "Tag "
                if action != CardEndActionType.tag:
                    message += "& suspend "
                message += str(len(lcWithClozeForEnd)) + " listening cards\n"

            if len(clozeCardsForStart) != 0:
                message += "Unsuspend " + str(len(clozeCardsForStart)) + " new cloze cards\n"

            if action == CardEndActionType.delete and len(lcWithoutClozeForEnd) != 0:
                message += "Delete " + str(len(lcWithoutClozeForEnd)) + " old listening cards without associated cloze card\n"
            ...
            if len(clozeCardsForEnd) != 0 and action != CardEndActionType.delete:
                ...
                if action != CardEndActionType.tag:
                    message += "& suspend "
                message += str(len(clozeCardsForEnd)) + " old cloze cards\n"
    ...
    def _getManagedRegularCardAndNoteIds(settings: Settings):
        listeningCardsBeyond = getListeningCardIds(mw.col, " -tag:" + CardManagement.lcTouchedTag + " prop:ivl>" + str(settings.lcEndInterval))
        listeningCardsUserSuspended = getListeningCardIds(mw.col, " is:suspended -tag:" + CardManagement.lcTouchedTag)
        ...
        for cardId in gcProcessedListeningCards:
            ...
            isBeginner = curLcCard.note().has_tag(TemplateNames.jlabBeginnerTag)
            clozeCards = getReadingCardIds(mw.col, " nid:" + str(curLcCard.nid))  # do not add attributes to search here
            if len(clozeCards) == 1:
        ...
        clozeCardsForEnd = getReadingCardIds(
            mw.col, " -tag:" + CardManagement.clozeTouchedTag + " -tag:" + TemplateNames.jlabBeginnerTag + " prop:ivl>" + str(settings.clozeEndInterval)
        )
    ...
    def garbageCollectionBeginner():
        ...
        allTaggedCards = findCards(mw.col, "tag:" + TemplateNames.jlabBeginnerTag)
        noteIds = set()
    ...
    def deactivateCardManagement():
        ...
        lcCards = getListeningCardIds(mw.col, " is:suspended -tag:" + TemplateNames.jlabBeginnerTag + " tag:" + CardManagement.lcUserTag)
        mw.col.sched.unsuspendCards(lcCards)

        newClozeCards = getReadingCardIds(mw.col, " is:suspended is:new")
        mw.col.sched.unsuspendCards(newClozeCards)

        oldClozeCards = getReadingCardIds(mw.col, " is:suspended -tag:" + TemplateNames.jlabBeginnerTag + " tag:" + CardManagement.clozeUserTag)
        mw.col.sched.unsuspendCards(oldClozeCards)

        mw.col.tags.bulk_remove(mw.col.find_notes("tag:jlabgc"), CardManagement.gcBaseTag)
