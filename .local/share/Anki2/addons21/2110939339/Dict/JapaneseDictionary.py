...
class JapaneseDictionary:
    def __init__(self, path=""):
        if len(path) == 0:
            path = os.path.dirname(os.path.dirname(os.path.dirname(__file__))) + "/Data/JDict.sqlite"
    ...
    def lookup(self, query):
        ...
        cursor.execute(
            "select JGroup.Expressions, JGroup.Readings, JGroup.Misc, JGroup.XRef, Sense.Gloss "
            "from JExpressions "
    ...
    def getAllReadings(self):
        cursor = self._db.cursor()
        cursor.execute(
            "select JGroup.Readings, JGroup.Id "
            "from JExpressions "
            "inner join JLookup on JLookup.JExpressionId = JExpressions.Id "
            "inner join JDict on JLookup.JDictId = JDict.Id "
            "inner join JGroup on JDict.JGroupId = JGroup.Id "
    ...
    def getExpressionAndReadingOfAllEntries(self):
        cursor = self._db.cursor()
        cursor.execute("select JGroup.Expressions, JGroup.Readings, JGroup.Misc " "from JGroup ")
