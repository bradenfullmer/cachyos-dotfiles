...
class NoteUpdater:
    ...
    def updateJlabNoteIds(self, collection):
        ...
        for cid in listeningCardIds:
            nids = collection.find_notes("cid:" + str(cid))
            if len(nids) != 1:
    ...
    def updateFieldsOnAllNotes(self, collection):
        ...
        if self._fastCardUpdate:
            listeningCardInReviewIds = getListeningCardIds(collection, " -is:new")
            ...
            listeningCardSuspendedIds = getListeningCardIds(collection, " is:suspended")
            ...
            clozeCardIds = getReadingCardIds(collection, " -is:new")
            for cid in clozeCardIds:
                nids = collection.find_notes("cid:" + str(cid))
                if len(nids) != 1:
                    raise Exception("Num notes for card != 1")
            ...
            starterDeckCards = findCards(collection, "tag:" + TemplateNames.jlabBeginnerTag + " -card:" + TemplateNames.infoTemplateName)
        ...
        progress = QProgressDialog("Updating cards...", "Cancel", 0, numNotes, mw)
    ...
    def _getDisplayText(self, hiraganaText, kanjiText, readingAssistanceType, cardCounter):
        if readingAssistanceType == ReadingAssistanceType.none:
            ...
            if (
                ...
                or self._kanjiTrainerData.displayType == KanjiTrainerData.DisplayType.furiganaWithSpace
            ):
                return furigana
            ...
            return self._kanaRoumajiTransliterator.transliterateCard(hiraganaText, "", False, True)
        elif readingAssistanceType == ReadingAssistanceType.latin:
            return self._kanaRoumajiTransliterator.transliterateCard(hiraganaText, "", False, False)
        else:
            raise ValueError("Not implemented")
